# 'SQLiteCreate_Database.py' converts the csv files (456.6GB) to an SQLite3 database 'washingtonstate.db' (255.2GB)
# Author: 
# Candace O'Sullivan(Sutherland) 3/21/2022 (Python 3.8.5)
# Cannabis Data Science Meetup group Keegan Skeate @ Cannlytics 
#
...
# set dir_in where dataset csv files are read from
CSV_DATA_DIR_IN = '/mnt/HDD'

# set dir_out where SQLite database lives
DB_DATA_DIR_OUT = "/mnt/USB12TB/washingtonstate.db"
...
for file_name in listDatasetsUnder10GB:
    ...
    if   file_name == 'Areas_0.csv': 
         ...
         data.to_sql('areas', washingtonstate_database, if_exists = 'append') 
    elif file_name  == 'LabResults_0.csv': 
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('labresults', washingtonstate_database, if_exists = 'append') 
         ...
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('labretests', washingtonstate_database, if_exists = 'append') 
    elif file_name  == 'Licensees_0.csv': 
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('licenses', washingtonstate_database, if_exists = 'append') 
    elif file_name  == 'MmeUser_0.csv': 
         print(f'WRITING to SQLite DB using file::', file_name)
         data.to_sql('mmeuser', washingtonstate_database, if_exists = 'append') 
    elif file_name  == 'Strains_0.csv': 
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('strains', washingtonstate_database, if_exists = 'append') 
    elif file_name  == 'Taxes_0.csv': 
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('taxes', washingtonstate_database, if_exists = 'append')          
    elif file_name  == 'Users_0.csv':
         print(f'WRITING to SQLite DB using file:', file_name)
         data.to_sql('users', washingtonstate_database, if_exists = 'append') 
    else:
...
for file_name in listDatasetsOver10GB:
    ...
    for chunk_df in pd.read_csv(f'{DATA_FILE_IN}', chunksize=chunksize, error_bad_lines=False, 
        ...
        if   file_name == 'Batches_0.csv': 
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('batches', washingtonstate_database, if_exists = 'append') 
        elif file_name  == 'Disposals_0.csv': 
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('disposals', washingtonstate_database, if_exists = 'append') 
        elif file_name  == 'Inventories_0.csv': 
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('inventories', washingtonstate_database, if_exists = 'append')    
        elif file_name  == 'InventoryAdjustments_0.csv': 
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('inventoryadjustments', washingtonstate_database, if_exists = 'append') 
            ...
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('inventorytypes', washingtonstate_database, if_exists = 'append') 
        elif file_name  == 'Plants_0.csv': 
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('plants', washingtonstate_database, if_exists = 'append')             
        elif file_name  == 'SaleItems_0.csv':
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('saleitems', washingtonstate_database, if_exists = 'append')                   
            ...
            print(f'WRITING to SQLite DB using file:', file_name)
            chunk_df.to_sql('sales', washingtonstate_database, if_exists = 'append') 
