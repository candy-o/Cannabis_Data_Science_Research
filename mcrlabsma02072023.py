"""
CoADoc | Get MCR MA Labs Test Result Data
...
Created: 7/13/2022
Updated: 2/7/2023 (added 5,6,-,8pg support to `parse_mcrlabs_pdf`)
    MCR LAB COA PDF page formats
    ...
    Page 6 of 6 page report is Terpenes Test results
    Page 6 of 8 page report and 7, 8 QC/QA (not used) 
Note: 
This is a standalone py and needs to be further integrated with Cannlytics
CONSTANTS.py and other py files before pulled into Cannlytics v13.
Latest v13 Cannlytics CONSTANTS.py needed to run standalone.
...
# It is assumed that there are the following analyses are on a 4 page COA.
""" MCR_LABS_COA_4pgs = {
    'fields': {
        'Sample ID #': 'lab_id',
...
# It is assumed that there are the following analyses are on a 5,6,8 page COA.
MCR_LABS_COA = {
    'fields': {
        'Sample ID #': 'lab_id',
        'Sample Name': 'product_name',
...
def parse_mcrlabs_pdf(
    ...
    # sample data differs 4pg "Serving size weight" 5,6,8 pages is "Sample Weight"
    ...
    for page in report.pages[1:]:
        ...
        if page.page_number < 7:
            ...
            if page.page_number == 5:
                analysis = 'pesticides'
                columns = [x.replace('Test Analysis', 'name').replace('Result, ppb', 'value').replace(' ppb', '').replace('Limits', 'Limit').replace('\n', '') for x in tables[0][0]]     

            elif page.page_number == 6:
                analysis = "terpenes"
                columns = [x.replace('Terpene', 'name').replace('Conc. (weight %)*', 'value').replace('\n', '') for x in tables[0][0]] 
            else:
                columns = [x.replace('\n', '') for x in tables[0][0]]
            ...
            # Get the values from the table 1 heavy_metals and append.
...
if __name__ == '__main__':
    ...
    # doc = '../../../tests/assets/coas/curaleafMA2022/MCRLabsMA6pg.PDF'
    # doc = './tests/assets/coas/curaleafMA2022/mcrlabsma6pgs.pdf'
    doc = './tests/assets/coas/mcr-labs/mcrlabsma4pgs.pdf'
    # doc = './tests/assets/coas/mcr-labs/mcrlabsma5pgs.pdf'    
    # doc = './tests/assets/coas/mcr-labs/mcrlabsma6pgs.pdf'
    # doc = './tests/assets/coas/mcr-labs/mcrlabsma8pgs.pdf'
    ...
    parser.save(data, "./4pgsMCRLabsMA02072023.xlsx")  # 
    # parser.save(data, "./5pgsMCRLabsMA02072023.xlsx")
    # parser.save(data, "./6pgsMCRLabsMA02072023.xlsx")
    # parser.save(data, "./8pgsMCRLabsMA02072023.xlsx")
